...
<div class="controls" ng-class="{'dock-left': dock.position == 'left', 
	...
	<ul class="nav nav-list shot-list">
		...
		<li ng-click="activeSession.setCurrentShot(shot)" ng-repeat="shot in activeSession.shots" ng-class="{active: activeSession.currentShot == shot}">
				 <a>
						<span class="fa" ng-click="shot.show = !shot.show" ng-class="{'fa-eye': shot.show, 'fa-eye-slash': !shot.show}"></span>
	...
	<div class="zoom-indicator input-prepend">
		...
		<input class="input-mini" readonly="readonly" type="text" ng-value="activeSession.viewport.zoom.level | percent">
	</div>
...
<div toolbelt="" ng-style="{height: activeSession.viewport.height, width: activeSession.viewport.width}" class="group-container">
	
	<div ng-repeat="shot in activeSession.shots" ng-style="{left: activeSession.viewport.left + (shot.screenPosition.x * activeSession.viewport.zoom.level), 
		top: activeSession.viewport.top + 41 + (shot.screenPosition.y * activeSession.viewport.zoom.level)}" class="shot-container" zoom="activeSession.viewport.zoom.level" ng-class="{'active-tab': activeSession.currentShot == shot, 'opacity-hide': !shot.show}">
		<canvas image-canvas="" class="screen-shot" animate-opacity="shot.screenAnimate" animate-time="shot.screenAnimateTime" src="shot.path" ng-style="{opacity: shot.screenOpacity}"></canvas>
	</div>
</div>
...
<div modal="uploader.open" ng-controller="uploaderCtrl" close="uploader.open = false" options="uploader.options">
	...
	<div class="modal-body">
      ...
      <tr ng-repeat="url in imageUrls">
		...
		<form>
      <input file-upload="onFileSelect($files)" accept=".bmp,.jpg,.jpeg,.gif,.png,.tiff" name="files[]" type="file" multiple="multiple" upload="">
      <div>- or -</div>
